COMPONENT(
  "toast",
  "timeout:8; position:top-right; loader:true; animate:fade",
  function (self, config) {
    self.singleton();
    self.readonly();
    self.template = Tangular.compile(
      '<div class="ui-toast {{ type }}" data-id="{{ id }}" {{ if callback }} style="cursor:pointer"{{ fi }}>{{ if loader }}<div class="loader"></div>{{fi}}<i class="fa fa-times"></i><div class="ui-toast-icon">{{if icon }}<i class="fa {{ icon }}"></i>{{fi}}{{if img }}{{ img | raw }}{{fi}}</div><div class="ui-toast-message">{{if date }}<div class="ui-toast-datetime">{{ date }}</div>{{fi}}{{ mess | raw }}</div></div>'
    );
    self.items = {};
    self.make = function () {
    ...
    self.append = function (mess, o, callback, tp, ic) {
      ...
      let img = o.img
        ? "<img class='img-rounded img-responsive' src='" + o.img + "'>"
        : null;
      let date = o.date
        ? o.date.format(config.format)
        : config.dateAlways
        ? new Date().format(config.format)
        : null;
...
function example(e) {
  ...
  if (e.data("img")) {
    options.img = "https://dummyimage.com/50x50/000/fff.jpg";
  }
  ...
  switch (e.data("type")) {
    case "success":
      toast.success(str[ind], options);
      break;
    case "warning":
      toast.warning(str[ind], options);
      break;
    case "error":
      toast.error(str[ind], options);
      break;
    case "info":
      toast.info(str[ind], options);
      break;
    default:
      toast.append(str[ind], options);
  }
}
